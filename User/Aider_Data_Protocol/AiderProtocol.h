...
#include "stdio.h" 
#define   MAX   20              //定义一帧数据最多包含的OID数量    
...
struct LiquidData 
{
	...
	float  CollectData[MAX];           //采集到的液位数据
	u16    DataCount;                  //采集到的数据总数
...
struct Config_RegPara 
{
	union 
	{
     u8       Data_Hex[4];            //液位报警阈值，存储的数据为Float类型对应的Hex数据，以方便Flash读写
		 float    Data_Float;
  }Threshold;
	union 
	{
     u8       Data_Hex[4];            //液位计探头安装高度，存储的数据为Float类型对应的Hex数据，以方便Flash读写
		 float    Data_Float;
  }MountHeight;	
...
typedef enum 
{
	...
	MOUNT_HEIGHT = 0x10000060,    //液位探头安装高度
	RESET_PROBER = 0x10000061,    //复位液位探头
	UPLOAD_CYCLE = 0x10000062,    //液位计数据上报周期
...
} CommandType;                  //液位计通信过程中使用到的OID集合（除了业务上报OID）
