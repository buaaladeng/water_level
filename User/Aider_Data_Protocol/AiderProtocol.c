...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
extern  struct LiquidSet  DeviceConfig;          //液位计配置信息结构体
...
void WakeupResponse(u8* pSendBuff, u8* pDeviceID, u16 NodeAddress)
{
  ...
  u8       SendCounter =1;     //发送次数计数器
	...
	WakeupSend.PDU_Type  =(0x0B<<8)+(1<<7)+7;
...
//	struct   DataFrame  StartupSend;
	...
	StartupSend.PDU_Type  =(8<<8)+(1<<7)+7;
  ...
  CrcData = CRC16(pSendBuff, j );   // Update the CRC value
	...
	for(i=0;i<SendCounter;i++)
	{
     RecevFlag =SendMessage(pSendBuff, j, pDeviceID, NodeAddress);
		 if(RecevFlag ==0x0987)     //成功接收到TrapResponse
		 {
        #if DEBUG_TEST	 
...
//	struct   SpecialDataFrame  TrapSend;
  ...
  LengthKey = LengthKey+6+TrapSend.SysTime.Width;
	...
	for(i=0,TrapSend.Tag_Count=0,TrapSend.Length =LengthKey ;i<pLevelData->DataCount ;i++)    //对每一个液位数据Tag赋值
	{
    TrapSend.TagList[i].OID_Data = (DeviceConfig.CollectPeriod<<11)+pLevelData->CollectTime[i] +(0xC3<<24);
		TrapSend.TagList[i].OID_Data = ntohl(TrapSend.TagList[i].OID_Data); //调整为网络序，即高字节在前，低字节在后
		TrapSend.TagList[i].Width =4;
		TrapSend.TagList[i].LevelData.Data_F =pLevelData->CollectData[i];  //采集到的液位数据
    
		TrapSend.Tag_Count++ ;                                             //对液位数据Tag进行计数，不包括采集日期Tag
...
//		 printf("\r\nReceive TrapResponse success:%4x!\r\n", RecevFlag); //测试使用
		 if(RecevFlag ==0x0587)     //成功接收到TrapResponse
		 {
        #if DEBUG_TEST	 
				printf("\r\nReceive TrapResponse success!\r\n"); //测试使用
				#endif
...
//	struct   DataFrame  GetRespSend;
	...
	GetRespSend.PDU_Type  =(2<<8)+(1<<7)+7;
		 ...
		 switch(RequestPara.OID_List[i])
	   {
        ...
        case CLT1_ITRL1:       //一时区采集间隔
            ...
            GetRespSend.TagList[i].OID_Command =CLT1_CNT1;
				...
				case MOUNT_HEIGHT :    //液位探头安装高度
				{
            GetRespSend.TagList[i].OID_Command =MOUNT_HEIGHT;
					  GetRespSend.TagList[i].Width =4;
					  UnionData.Data_Float =DeviceConfig.MountingHeight;              //给共用体变量赋值，便于做格式转换
					  GetRespSend.TagList[i].Value[0] = UnionData.Data_Hex[0];        //探头安装高度，浮点型，低字节在前
					  GetRespSend.TagList[i].Value[1] = UnionData.Data_Hex[1];   
            GetRespSend.TagList[i].Value[2] = UnionData.Data_Hex[2];
            GetRespSend.TagList[i].Value[3] = UnionData.Data_Hex[3];   					
					  GetRespSend.Tag_Count++;
					  GetRespSend.Length =GetRespSend.Length  + GetRespSend.TagList[i].Width +6; 
					  break;
        }	
  ...
  CrcData = CRC16(pSendBuff, j );   // Update the CRC value
	...
	for(i=0;i<SendCounter;i++)
	{
     RecevFlag =SendMessage(pSendBuff, j, pDeviceID, NodeAddress);
		 ...
		 if((RecevFlag ==0x0187)||(RecevFlag ==0x0387)||(RecevFlag ==0x0087))     //成功接收到GetResponse
		 {
        #if DEBUG_TEST	 
