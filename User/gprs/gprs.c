  ...
  u16  TempShort =0x0000;        //短整型临时变量
	...
	switch(ParaRequest)
	{
       ...
       printf("\r\n UploadCycle:%2x\r\n",TempShort);	                      //测试使用		 
		...
		case MOUNT_HEIGHT:            //存储探头安装高度
		{
			 DataWrite_To_Flash(0,5,0,(u8*)pSetPara, ParaLen);                    //将探头安装高度写入Flash ，低地址对应低字节
       printf("\r\n MountingHeight:%f\r\n",*(float*)pSetPara);	            //测试使用
			 return 1;
		}
                            ...
                            Time_Auto_Regulate(&DeviceConfig);             //通过服务器下发参数进行RTC时钟校准，
                             ...
                             UploadFlash((u8*)&(DeviceConfig.CollectNum), 2, CLT1_CNT1);  //参数存入Flash
											...
											case MOUNT_HEIGHT :    //液位探头安装高度
											{
													for(j=0;j<4;j++)
												  {
                             UnionData1.Data_Hex[j] =pChar[6+j];          //顺序有待测试
                          }
												  DeviceConfig.MountingHeight =UnionData1.Data_Float;
													printf("\r\n-MountingHeight:-%f---.\r\n", DeviceConfig.MountingHeight );         //测试使用
													if(DeviceConfig.MountingHeight>=0)                     //参数合法性判定
													{
                             UploadFlash(UnionData1.Data_Hex, 4, MOUNT_HEIGHT);  //参数存入Flash
                          }
												  ParaRequest.OID_List[i] =MOUNT_HEIGHT;           
...
////////////////////////////////////////////////////////////////////////////////////////////		
		DataRead_From_Flash(0,5,0, ConfigData.MountHeight.Data_Hex,4);   //从Flash中读取预设服务器IP		//	length = strlen(ConfigData.SMS_Set_ServerIP);
		fTemp = ConfigData.MountHeight.Data_Float;
		if((fTemp>0.5)&&(fTemp<=10))                          //安装高度下限和上限根据探头量程、盲区以及天线固定方式而定	
		{
			 Para->MountingHeight =fTemp;      
		}
		else                                                     
		{ 
			 Para->MountingHeight =0;                           //读取数据无效时，安装高度设为0，当安装高度为0时，上传液位数据为探头距离液面的高度，而不是液面的绝对深度
		}
/////////////////////////////////////////////////////////////////////////////////////////////	
...
/////////////////配置参数约束条件判断///////////////////////////////////
  if(DeviceConfig.CollectNum * DeviceConfig.CollectPeriod >DeviceConfig.UploadCycle)
	...
	if(DeviceConfig.CollectNum >5)
	{
    DeviceConfig.CollectNum =5;           //由于数据发送以及数据存储空间的限制，数据采集次数必须限制在5以内
  }
/////////////////////////////////////////////////////////////////////////////////////////////	
		Para->Time_Sec  =0x00;
		Para->Time_Min  =0x00;
